...
public class Visualization extends Canvas {
    ...
    PcaDataPoint mean;
    double scaleOfFirstEigenvector = 1.0;
    double scaleOfSecondEigenvector = 0.0;
    double scaleOfThirdEigenvector = 0.0;

    public Visualization(EigenDecomposition ed, PcaDataPoint mean) {
        this.ed = ed;
        this.mean = mean;
    }
    ...
    public static Visualization start(EigenDecomposition ed, PcaDataPoint mean) {
        ...
        frame.setMinimumSize(new Dimension(1500, 1100));
        Visualization visualization = new Visualization(ed, mean);
        visualization.setSize(1500, 1100);
    ...
    public void paint(Graphics g) {
        ...
        RealVector scaledFirstEigenvector = ed.getEigenvector(0).mapMultiply(scaleOfFirstEigenvector);
        RealVector scaledSecondEigenvector = ed.getEigenvector(1).mapMultiply(scaleOfSecondEigenvector);
        RealVector scaledThirdEigenvector = ed.getEigenvector(2).mapMultiply(scaleOfThirdEigenvector);
        PcaDataPoint pointToDraw = mean.add(scaledFirstEigenvector);
        pointToDraw = pointToDraw.add(scaledSecondEigenvector);
        pointToDraw = pointToDraw.add(scaledThirdEigenvector);
