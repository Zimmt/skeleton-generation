...
Damit können dann die Drehmomente der einzelnen Knochen um den Schwerpunkt berechnet wrden. Addieren sie sich alle zu null auf, ist das Skelett im Gleichgewicht.

% Probleme
Hierbei tauchen gleich mehrere Fragen auf:
\begin{description}
 \item[Gewicht] Wie wird das Gewicht der Knochen bestimmt? Hängt es von der Größe der Knochen ab? Außerdem müsste das ganze Gewicht des Tieres, nicht nur das der Knochen berücksichtigt werden. Wie kann bestimmt werden wieviel anderes Gewebe an einem Knochen hängt?
 
 \item[Schwerpunkt] Wo liegt der Schwerpunkt? Wird am Anfang ein Quader festgelegt, in dem das Skelett liegen muss und dessen Mittelpunkt der Schwerpunkt des Skeletts liegt? Verändert der Schwerpunkt seine Position je nach dem welche Knochen schon generiert wurden?
 
 \item[Gleichgewicht] Wann wird überprüft ob sich das Skelett im Gleichgewicht befindet? Ist das Gleichgewicht eine Invariante, die während der Generierung aufrecht erhalten wird? Oder wird es erst am Ende geprüft? Was passiert, wenn sich das Skelett nicht im Gleichgewicht befindet?
\end{description}
 
% Warum wird anderer Ansatz verwendet
Das Hauptproblem ist, dass nicht klar ist wie das Gewicht der einzelnen Körperteile bestimmt werden soll ohne zusätzlich auch noch anderes Gewebe, wie Muskeln oder Eingeweide, zu betrachten. Da sich diese Arbeit aber nur auf das Skelett beschränken soll, wurde ein anderer Ansatz erarbeitet.
...
\begin{figure}
 ...
 Um zu verstehen in welchem Bereich sich die zufällig generierten Punkte bewegen, ist es sinnvoll sich anzuschauen was die \emph{principal components} "`bedeuten"'. In Abbildung \ref{pca_results_sqrtEV_biggest} sind zwei Datenpunkte zu sehen, bei denen jeweils alle Dimensionen auf null festgelegt sind, nur der Wert für die Dimension des größten Eigenvektors ist die positive \bzw negative Standardabweichung. Die Varianz entlang eines Eigenvektors ist gegeben durch den Eigenwert. Die Standardabweichung ist also die Wurzel des entsprechenden Eigenwerts. Es ist zu sehen wie Punkte entlang des ersten Eigenvektors verändert werden \bzw welchen Einfluss er ausübt. Abbildungen für die sechs größten Eigenvektoren sind im Anhang in Abbildung \ref{pca_results_sqrtEV} zu finden.
 ...
 \begin{figure}
   ...
   \caption{Datenpunkte im PCA-Koordinatensystem. Die Koordinate für den größten Eigenvektor nimmt den Wert der positiven \bzw negativen \mbox{Standardabweichung $\sigma$} an, alle anderen sind null. Für Abbildungen zu den größten sechs Eigenvektoren siehe Abbildung \ref{pca_results_sqrtEV} im Anhang}
