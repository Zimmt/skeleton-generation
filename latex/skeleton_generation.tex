 ...
 \begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule} % fbox has more width than content
  ...
  \begin{minipage}{0.9\textwidth}
    ...
    \begin{enumerate}
      ...
      \item Bestimme einen Punkt $p$ im PCA-Raum und dessen Grundlage die Parameter für die Grammatik $G$.
...
\begin{alignat*}{2}
 ...
 N &= \{&&\text{\emph{Skelett, Vorderteil, Hinterteil, Schulterpartie, Schultergürtel, Beckengürtel}}\\
        & &&\text{\emph{Hals, Vorderextremität, Hinterextremität}}\}\\
 S &= &&\text{\emph{Skelett}}\\
...
\begin{align*}
 ...
 \text{\emph{Vorderteil}} \rightarrow &\text{ Wirbel}^{(w_{rv} - r)^+}\\
    ...
    &\text{ Wirbel ( \emph{Schulterpartie} )}\\
    &\text{ \emph{Hals}}\\
...
Als Grundlage für diese Entscheidungen dient der vorher gewählte Punkt $p$ (siehe Überblick \ref{section:overview}).
...
\begin{itemize}
 ...
 \item Ob überhaupt eine Hals- \bzw Schwanzwirbelsäule generiert wird hängt davon ab, ob dieser Teil der Wirbelsäule in der Wirbelsäule vorkommt, die durch $p$ vorgegeben wird.
...
Die Produktion in $P'$ \bzw $P$ für den \emph{Hals} wird in $P''$ durch folgende Produktion ersetzt.
...
Wie an der Schulter, werden auf dem Hals, mit dem Abstand von einem Wirbel, zwei Schultergürtel erzeugt. Die Gesamtzahl der Wirbel bleibt dabei gleich.  Dies funktioniert auch problemlos, da der Hals, falls vorhanden, immer $\geq 7$ Wirbel besitzt.
...
Für die Generierung von $n$ Wirbeln wurde die Kurve einfach an den Stellen $\frac{i}{n}$ ausgewertet, für $0 \leq i \leq n$. Die Länge des $i$-ten Wirbels ergibt sich dann einfach aus den Abstand zwischen den Stellen $\frac{i-1}{n}$ und $\frac{i}{n}$ auf der Bézierkurve. Durch den oben genannten Effekt variiert dann die Länge der Wirbel über den Kurvenverlauf. Die Wirbel werden also auf der Bézierkurve aufgereit und so positioniert, dass sie an den ausgewerteten Stellen aneinander stoßen.
...
Deshalb wird die Anzahl der Wirbel $r$ zufällig bestimmt aus dem Intervall $ [0, n]$, mit $n =$ Anzahl der Rückenwirbel, bestimmt.
