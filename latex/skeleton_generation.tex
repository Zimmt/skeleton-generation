...
%-----------------------------
\section{Aufbau der Regeln zu Generierung}
\label{section:grammar}

% erweiterte kontextfreie Grammatik
Um ein Skelett zu generieren ist ein Regelwerk notwendig. Die Grundidee war zunächst dazu eine kontextfreie Grammatik zu verwenden. Jedoch können kontextfreien Grammatiken nur Wörter erzeugen, keine Strukturen, die sich verzweigen. Verzweigungen sind aber nötig um die Baumstruktur des Skeletts, wie im Grundbauplan \ref{bauplan_skelett}, darzustellen.

% Darstellung als Graph
Um Bäume zu erzeugen, könnten L-Systemen (siehe Abschnitt \ref{grammars}) mit Verzweigungen verwendet werden.
Letztendlich wären die Produktionen der Grammatik jedoch so aufgebaut, dass jedes Nichtterminalsymbol genau einmal vorkommt und genau einmal die entsprechende Produktionsregel angewandt wird. Deshalb ist es einfacher sich die erzeugte Struktur direkt als Baum, also als gerichteten, zusammenhängenden Graphen ohne Kreise, vorzustellen.

\paragraph{Darstellung als Baum}
% Definition
Der Baum $G = (V, E)$ hat die Knotenmenge $V$ und die Kantenmenge $E$. Die Knoten können \emph{terminal} oder \emph{nichtterminal} sein. Nichtterminale Knoten können keine Kindknoten haben, sind also Blätter (dazu weiter unten mehr). Für jeden nichtterminalen Knoten $v$ gibt es genau eine Ersetzungsregel, die $v$ durch einen Baum $B_v$ ersetzt. In diesem Prozess wird $v$ aus $G$ gelöscht und alle Knoten und Kanten von $B_v$ in $G$ eingefügt. Außerdem wird eine Kante zwischen der Wurzel von $B_v$ und dem Elternknoten von $v$ eingefügt, falls $v$ nicht die Wurzel von $G$ war. Wenn $G$ vor der Anwendung der Ersetzungsregel ein Baum war, so ist $G$ nach der Ersetzung also immernoch ein Baum.

% konkreter Aufbau
Die Skelettgenerierung startet mit einem Graphen $G$, wie oben, der nur aus einem einzelnen initialen nichtterminalen Knoten besteht. Der initiale Graph $G$ ist also ein Baum.
...
% Nichtterminale immer Blätter
Terminale Knoten stehen für die Knochen des Skeletts. Sie haben also, wie in Abschnitt \ref{skeleton_parts} schon beschrieben, eine Position und eine Ausdehnung. Nichtterminale Knoten hingegen haben diese Eigenschaften nicht. Sie sind nur Platzhalter für noch zu generierende Knochen. Deshalb gibt es die Einschränkung, dass nichtterminale Knoten nur Blätter des Baums sein dürfen. Hätten sie nämlich terminale Kindknoten, so wäre nicht klar wie diese positioniert werden sollen.

\paragraph{Variation der Ersetzungsregeln}
% Randbedingungen für Ersetzungsregeln
Die Ersetzungsregeln sind zwar für jedes Skelett die gleichen, dennoch berücksichtigen sie bei der Erzeugung der neuen Knoten verschiedene Randbedingungen. Dies sind vor allem die Merkmale des gewählten Punkts im PCA-Raum, \zb der Verlauf der Wirbelsäule oder die Länge der Extremitäten. Andere Bedingungen werden zufällig bestimmt, wie \zb die Anzahl der Wirbel und Rippen auf der Wirbelsäule.

% optionale Elemente
Außerdem sind, wie auch schon beim Grundbauplan \ref{bauplan_skelett}, manche Teile des Skeletts optional. Gegebenenfalls werden also nichtterminale Knoten, nur durch Teilmengen der hier angegebenen Knoten ersetzt. Schulterblatt und Beckenknochen werden beispielweise nur dann generiert, wenn es auch Extremitäten gibt, die daran ansetzen sollen. Der Schultergürtel ist Kindknoten der Rippe am Schulterwirbel. Existiert diese Rippe jedoch nicht, ist er direkter Kindknoten des Schulterwirbels. Bei allen anderen Knoten gilt: fehlt das Elternelement, so wird dieser Knoten nicht eingefügt.

\todo{genauere Beschreibung der Regeln nötig?}
