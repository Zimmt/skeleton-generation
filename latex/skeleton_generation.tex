...
\begin{alignat*}{2}
 ...
 N &= \{&&\text{\emph{Skelett, Vorderteil, Hinterteil, Schulterpartie, Schultergürtel, Hüftgürtel}}\\
        & &&\text{\emph{Halswirbelsäule, Schwanzwirbelsäule, Vorderextremität, Hinterextremität}}\}\\
 S &= &&\text{\emph{Skelett}}\\
 p &= \{&&w_{rv}, w_{rh}, w_h, w_s, r, s, b \}
\end{alignat*}
...
\begin{align*}
 ...
 \text{\emph{Vorderteil}} \rightarrow &\text{ Wirbel}^{(w_{rv} - r)^+}\\
    ...
    &\text{ \emph{Schulterpartie}}\\
    &\text{ \emph{Halswirbelsäule}}^h\\
    &\text{ Schädel}\\
 \text{\emph{Schulterpartie}} \rightarrow &\text{ [Wirbel ( Rippe$^{\text{min}(1,r)}$ \emph{Schultergürtel}$^{e_v}$ )}\\ 
 ...
 \text{\emph{Halswirbelsäule}} \rightarrow &\text{ Wirbel}^{w_h}\\
 \text{\emph{Hinterteil}} \rightarrow &\text{ [Wirbel ( Rippe )]}^{(r-w_{rv})^+}\\
    &\text{ Wirbel}^{(w_{rh} - (r-w_{rv})^+ - 1)^+}\\
    &\text{ Wirbel ( \emph{Hüftgürtel} )}\\
    &\text{ \emph{Schwanzwirbelsäule}}^s\\
 \text{\emph{Hüftgürtel}} \rightarrow &\text{ [Beckenknochen \emph{Hinterextremität} ]$^{e_h}$}\\
 \text{\emph{Hinterextremität}} \rightarrow &\text{ Oberschenkel Unterschenkel Fuß}\\
 \text{\emph{Schwanzwirbelsäule}} \rightarrow &\text{ [Wirbel]}^{w_s}\\
\end{align*}

Der Ausdruck $A^x$ mit $A \in (\Sigma \cup N)^*$ bedeutet, dass $A$ genau $x$-mal hintereinander vorkommt. Der Ausdruck $A^{(x)^+}$ kann ersetzt werden durch $A^{\text{max}(0, x)}$. Die angegebene Anzahl $x$ wird also nur verwendet, wenn sie positiv ist. Sonst wird $x$ durch $0$ ersetzt.
...
\begin{itemize}
 ...
 \item $w_h$: Anzahl der Wirbel auf der Halswirbelsäule
...
\end{itemize}
\todo{wie werden Parameter bestimmt}
...
Deshalb wird zusätzlich zur Bodenhöhe noch eine Wahrscheinlichkeit berechnet, dass Ferse \bzw Handgelenk den Boden berühren.
...
Um das zu ermöglichen, wird die, in Abschnitt \ref{section:grammar} eingeführte, Grammatik $G = (\Sigma, N, S, P, p)$ zu einer Grammatik $G' = (\Sigma', N, S, P', p')$ mit $\Sigma' = \Sigma \cup \{\text{Beckenknochen2}\}$ erweitert. Das Terminal Beckenknochen2 steht hier für einen Beckenknochen mit zwei Gelenken für Extremitäten. Die Produktionen in $P$ für \emph{Vorderteil, Schulterpartie} und \emph{Beckengürtel} werden in $P'$ durch folgende Produktionen ersetzt. Außerdem können die Parameter $e_v$ und $e_h$ (die Anzahl der Vorder- \bzw Hinterextremitäten) in $p'$ jetzt auch den Wert $2$ annehmen.

\begin{align*}
 \text{\emph{Vorderteil}} \rightarrow &\text{ Wirbel}^{(w_{rv} - r)^+}\\
    &\text{ [Wirbel ( Rippe )]}^{(\text{min}(w_{rv}-\mathbf{3}, r-\mathbf{3}))^+}\\
    &\text{ \emph{Schulterpartie}}\\
    &\text{ \emph{Halswirbelsäule}}^h\\
    &\text{ Schädel}\\
 \text{\emph{Schulterpartie}} \rightarrow &\text{ [Wirbel ( Rippe$^{\text{min}(1,r-2)}$ \emph{Schultergürtel}$^{(e_v-1)^+}$ )}\\
    &\text{ Wirbel ( Rippe$^{(\text{min}(1, r-1))^+}$ )}\\
    &\text{ Wirbel ( Rippe$^{\text{min}(1,r)}$ \emph{Schultergürtel}$^{\text{min}(1, e_v)}$ )}\\   
 \text{\emph{Hüftgürtel}} \rightarrow &\text{ [Beckenknochen \emph{Hinterextremität}]$^{\text{min}(1, e_h) \cdot |e_h - 2|}$}\\
    &\text{ [Beckenknochen2 ( \emph{Hinterextremität} ) \emph{Hinterextremität}]$^{(e_h - 1)^+}$}
\end{align*}
\todo{Regeln nochmal textuell erklären}
...
Hierfür wird die Grammatik $G' = (\Sigma', N, S, P', p')$ zu einer Grammatik $G'' = (\Sigma', N, S, P'', p'')$ mit $p'' = p' \cup \{e_{v2}\}$ erweitert. Der Parameter $e_{v2} \in \{0, 1, 2\}$ steht für die Anzahl der  Vorderextremitäten am zweiten Schultergürtel.
Die Produktion in $P'$ \bzw $P$ für die \emph{Halswirbelsäule} wird in $P''$ durch folgende Produktion ersetzt.

\begin{align*}
 \text{\emph{Halswirbelsäule}} \rightarrow &\text{ Wirbel}^{(w_h - 5)^+}\\
    &\text{ Wirbel ( \emph{Schultergürtel}$^{(e_{v2}-1)^+}$ )}\\
    &\text{ Wirbel}\\
    &\text{ Wirbel ( \emph{Schultergürtel}$^{\text{min}(1, e_{v2})}$ )}\\
    &\text{ Wirbel$^2$}\\
\end{align*}
\todo{Regel nochmal textuell erklären}
