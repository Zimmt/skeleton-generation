 ...
 \begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule} % fbox has more width than content
  ...
  \begin{minipage}{0.9\textwidth}
    ...
    \begin{enumerate}
      ...
      \item Generiere ein 3D-Modell, das alle generierten Elemente enthält.
    \end{enumerate}
    \vspace{0.2cm}
...
Ist ein Elternknochen mit einem Gelenk in einer bestimmten Ausrichtung gegeben, so lässt sich damit die Transformationsmatrix des Kindelements berechnen.
...
Das Regelwerk für die Generierung der Knochen des Skeletts ist durch eine kontextfreie Grammatik gegeben (siehe \ref{grammars}).

\paragraph{Kontextfreie Grammatik}
% erweiterte kontextfreie Grammatik
Die verwendete kontextfreie Grammatik $G = (\Sigma, N, S, P)$ unterstützt Klammerausdrücke, um die Baumstruktur des Skeletts darzustellen (ähnlich wie sie auch bei L-Systemen verwendet werden, siehe dazu Abschnitt \ref{grammars}).
\begin{alignat*}{2}
...
Bevor die Grammatik verwendet wird, müssen die Zusatzbedingungen in den Produktionen spezifiziert werden. Bei Ausdrücken $A^x$ muss bestimmt werden wie oft $A$ vorkommt und bei $A?$ muss bestimmt werden ob $A$ vorkommt.
...
Ein Punkt im PCA-Raum gibt schon viele Eigenschaften des zu generierenden Skeletts vor. Zu den Dingen, die noch festgelegt werden müssen, zählen vor allem die Anzahl und Ausrichtung der Extremitäten. 
...
Je nach Art wird die Anzahl der entsprechenden Extremitäten unterschiedlich bestimmt. Als Grundlagen dienen die Werte $w_b$ und $w_f$, die der gewählte Punkt $p$ im PCA-Raum für \emph{Beine mit Bodenkontakt} und \emph{Flügel} annimmt. Priorisiert werden die verschiedenen Arten in der Reihenfolge, wie sie im Folgenden aufgezählt sind.

\begin{itemize}
 \item Beine: Der Wert $w_b$ liegt im Intervall $[0, 2]$ \bzw wenn $w_b < 0$ oder $w_b > 2$, so wird $w_b$ auf $0$ \bzw $1$ festgelegt. Gilt $w_b \in [0, 1]$, so wird mit Wahrscheinlichkeit $w_b$ ein Hinterbein und kein Vorderbein generiert. Gilt $w_b \in [1, 2]$, so wird mit Wahrscheinlichkeit $1$ ein Hinterbein und mit Wahrscheinlichkeit $w_b - 1$ ein Vorderbein generiert.
...
Hierfür wird die Grammatik aus Abschnitt \ref{section:grammar} um zwei zusätzliche Produktionen für das \emph{Vorderteil} erweitert:
